...
def read_updates():
    """Parse content/updates.md into a list of update dicts (newest first).

    One update per line, pipe-delimited:
        YYYY-MM-DD | English text | Urdu text | optional-url
    Urdu and url are optional. Lines starting with # and blank lines are ignored.
    """
    ...
    for raw in path.read_text(encoding="utf-8").splitlines():
        ...
        en = parts[1]
        ur = parts[2] if len(parts) > 2 and parts[2] else en
        url = parts[3] if len(parts) > 3 and parts[3] else ""
        out.append({"date": date, "date_human": date.strftime("%d %b"),
                    "en": en, "ur": ur, "url": url})
...
def build(retranslate=False, no_translate=False):
    ...
    for path in sorted(CONTENT.glob("*.md")):
        if path.name in ("updates.md",):   # reserved: the live-updates feed, not an article
            continue
    ...
    # Live-updates rail (curated). Reads content/updates.md if present.
    T.set_updates(read_updates())

    # Social share cards ("placards" shown on X / WhatsApp / Facebook).
    # Entirely optional: if Pillow or a usable font is missing, every call
    # returns None, pages fall back to /static/og-default.png, and the build
    # carries on. A share image must never be able to break publishing.
    import ogcards
    if ogcards.available():
        SITE["og_default"] = ogcards.build_default_card(SITE, DIST)
        made = 0
        for a in articles:
            a["og_image"] = ogcards.build_article_card(a, SITE, DIST)
            made += 1 if a["og_image"] else 0
        print(f"Share cards: {made}/{len(articles)} generated")
    else:
        print("Share cards: skipped (Pillow/font unavailable) - using default image")
