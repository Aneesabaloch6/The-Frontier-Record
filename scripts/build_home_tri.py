#!/usr/bin/env python3
"""homepage, archive, topic pages — news-portal shell from tribuilder.py."""
...
def _home_crumb():
    return [{"en": UI["home"]["en"], "ur": UI["home"]["ur"], "href": None}]


def _sub_crumb(en, ur):
    return [{"en": UI["home"]["en"], "ur": UI["home"]["ur"], "href": "/"},
            {"en": en, "ur": ur, "href": None}]
...
{more}"""
    (dist / "index.html").write_text(
        T.shell(inner, site["title"], site["description"], site, year, nav_topics,
                active="home", crumb=_home_crumb()), encoding="utf-8")
...
<section class="entries">{entries}</section>"""
    (dist / "archive.html").write_text(
        T.shell(inner, f"Archive \u2014 {site['title']}", site["description"], site, year,
                nav_topics, active="archive", crumb=_sub_crumb("Archive", "آرکائیو")),
        encoding="utf-8")


def build_topics(topics_index, site, dist: Path, year, nav_topics):
    out = dist / "topics"; out.mkdir(parents=True, exist_ok=True)
...
<section class="entries">{entries}</section>"""
        (out / f"{slug}.html").write_text(
            T.shell(inner, f"{data['en']} \u2014 {site['title']}", site["description"], site, year,
                    nav_topics, active=slug, crumb=_sub_crumb(esc(data['en']), esc(data['ur']))),
            encoding="utf-8")
