...
UI = {
    ...
    "updates":  {"en": "Live Updates", "ur": "تازہ اپڈیٹس"},
}
...
# Live-updates rail. build_from_markdown sets this once via set_updates().
_UPDATES: list[dict] = []


def set_updates(items: list) -> None:
    global _UPDATES
    _UPDATES = items or []


def updates_rail() -> str:
    """Left-column 'Live Updates' panel. Empty string if no updates are set."""
    if not _UPDATES:
        return ""
    items = []
    for u in _UPDATES:
        label = bi(esc(u["en"]), esc(u["ur"]))
        body = (f'<a href="{esc(u["url"])}">{label}</a>' if u.get("url") else label)
        items.append(
            f'<li class="ur-item"><time class="ur-date">{esc(u["date_human"])}</time>'
            f'<span class="ur-text">{body}</span></li>')
    return (f'<aside class="updates-rail" aria-label="Latest updates">'
            f'<div class="ur-head"><span class="ur-dot"></span>'
            f'{bi(UI["updates"]["en"], UI["updates"]["ur"])}</div>'
            f'<ul class="ur-list">{"".join(items)}</ul></aside>')
...
def shell(inner: str, page_title: str, description: str, site: dict,
          ...
          og_type: str = "article") -> str:
    rail = updates_rail()
    cls = "container has-rail" if rail else "container"
